$(function() {
	...
	var action = {
        ...
        add : function() {
            ...
            var data = new Object();
            data.channel_id = $("#input-channelid").val();
            data.app_id = $("#input-appid").val();
		...
		//编辑数据
		edit : function() {
            var url = ctx + "xiaoyusvr/app/channellist/update";
            var data = new Object();
    ...
    $("#pageContent").on("click",".table-edit-btn",function(){
        ...
        $("#input-channelid").val(that.find("td").eq(1).text());
        $("#input-appid").val(that.find("td").eq(2).text());
	...
	$("#addTempl-modal").on('show.bs.modal', function(e) {
		...
		if (!e.relatedTarget) {
			$("h4#addTempl-modal-label").text("编辑频道应用");
			$form.data("action", "edit");
		} else if (e.relatedTarget.id = "btn-add") {
            $("h4#addTempl-modal-label").text("添加频道应用");
    ...
    $("#input-search-appid").on('keydown', function(e) {
        if (e.keyCode == 13) {
            action.loadPageData();
        }
    });
