$(function() {
	...
	var action = {
        ...
        //新增数据
        add : function() {
            var url = ctx + "xiaoyusvr/sensitiveword/insert";
            var data = new Object();
            data.sensitive_word = $("#input-sensitiveword").val();
        ...
        deleteConfig : function(id) {
            if (confirm("删除后不可恢复，确定删除" + name + "？")) {
                var url = ctx + "xiaoyusvr/sensitiveword/delete";
                var data = new Object();
                data.id = parseInt(id);
    ...
    $("#pageContent").on("click",".table-edit-btn",function(){
        ...
        $("#input-channelname").val(that.find("td").eq(5).text());
        $("#input-background").val(that.find("td").eq(6).text());
        $("#input-description").val(that.find("td").eq(1).text());
        $("#input-prority").val(that.find("td").eq(3).text());
        $("#input-textcolor").val(that.find("td").eq(7).text());
	...
	$("#addTempl-modal").on('show.bs.modal', function(e) {
		...
		if (!e.relatedTarget) {
			$("h4#addTempl-modal-label").text("编辑敏感词");
			$form.data("action", "edit");
		} else if (e.relatedTarget.id = "btn-add") {
            $("h4#addTempl-modal-label").text("添加敏感词");
	...
	$("#input-search-sensitiveword").on('keydown', function(e) {
        if (e.keyCode == 13) {
            action.loadPageData();
        }
	});
