...
public class ProblemController {
	...
	public Response insertProblem(@RequestBody @Valid Problems record) throws IOException {
		...
		/*
		 * if(record.getSchoolId()==null){ return
		...
		/*
		 * MultipartFile file=null; String pictureURL = "";
		 * 
		 * //上传图片文件 if(uploadfile.length!=0){ for(int i=0;i<uploadfile.length;i++){
		 * file=uploadfile[i];
		 * 
		 * String path=req.getSession().getServletContext().getRealPath("upload");
		 * String filename=file.getOriginalFilename(); File dir=new File(path,filename);
		 * if(!dir.exists()){ dir.mkdirs(); }
		 * 
		 * file.transferTo(dir); if(i==0){ pictureURL=path+"/"+filename+new
		 * Random().nextInt(16); }else { pictureURL=pictureURL+","+path+"/"+filename+new
		 * Random().nextInt(16); }
		 * 
		 * } }
		 * 
	 ...
	 * @IgnoreAuth
	 * 
	 * @RequestMapping(value="/queryall",method=RequestMethod.POST) public Response
	 * queryProblem(@RequestBody @Valid Page<Problems> param){ Response res = new
	 * Response(); Response res1 = new Response(); Concern record=new Concern();
	 * logger.info(param.toString());
	 * 
	 * if(param.getParam().getSchoolId()==null ||
	 * param.getParam().getUserId()==null){ return
	 * res.failure(ErrorCode.ERROR_FISE_DEVICE_PARAM_NULL); }
	 * 
	 * res=problemService.queryAll(param);
	 * 
	 * //添加判断用户是否关注该问题 Page<ProResult> page=(Page<ProResult>)res.getData();
	 * List<ProResult> list=page.getResult(); for(int i=0;i<list.size();i++){
	 * record.setProblemId(list.get(i).getId());
	 * record.setUserId(param.getParam().getUserId());
	 * res1=concernService.queryisConcern(record);
	 * list.get(i).setIsConcern(res1.getMsg()); } return res;
	 * 
	 * }
	 */
	...
	public void filedown(@RequestBody Map<String, String> map, HttpServletResponse resp) {
