$(function() {
	...
	var action = {
                    ...
                    success:function(result){
                        if (result.ret == true) {
                            ...
                            if (++imgCount == imgLen){
                                ...
                                var url = ctx + "xiaoyusvr/appsplash/insert";
                                var data = new Object();
                                data.status = parseInt($("input[name=status]:checked").val());
                                data.name = $("#input-name").val();
                                data.url = imgStr;
                                data.action_url = $("#input-action_url").val();
                                data.prority = parseInt($("#input-prority").val());
                                data.delay = $("#input-delay").val();
		...
		//编辑数据
		edit : function() {
            var url = ctx + "xiaoyusvr/appsplash/update";
            var data = new Object();
    ...
    $("#pageContent").on("click",".table-edit-btn",function(){
        ...
        if(check_status === "弃用"){
            status_val = 0;
        }else if(check_status === "启用"){
            status_val = 1;
        }

        $("#input-id").val(that.find("td").eq(0).text());
        $("#input-name").val(that.find("td").eq(1).text());
        $("#input-url").val(that.find("td").eq(2).text());
        $("#input-action_url").val(that.find("td").eq(3).text());
        $("#input-prority").val(that.find("td").eq(4).text());
        $("#input-delay").val(that.find("td").eq(6).text());
        $("input[name=status]").filter("[value=" + status_val + "]").prop('checked', true);

        var imgList = $.trim(that.find("td").eq(2).text()).split(";");
	...
	$("#addTempl-modal").on('show.bs.modal', function(e) {
		...
		if (!e.relatedTarget) {
			$("h4#addTempl-modal-label").text("编辑闪屏页信息");
            $("#addImgUrl").hide();
            $("#showImgUrl").show();
            $("#showPic").show();
			$form.data("action", "edit");
		} else if (e.relatedTarget.id = "btn-add") {
            $("h4#addTempl-modal-label").text("添加闪屏页信息");
            $("#addImgUrl").show();
            $("#showImgUrl").hide();
            $("#showPic").hide();
